...
app.config['MONGO_URI'] = 'mongodb://localhost:27017/metapython_db'
mongo = PyMongo(app)
...
def insert_log(texto):
    log = {
        'fecha_y_hora': datetime.utcnow(),
        'texto': texto
    }
...
@app.route('/')
def index():
    registros = list(mongo.db.logs.find())
...
def addMessageLog(texto):
    ...
    mongo.db.logs.insert_one(newRegister)
